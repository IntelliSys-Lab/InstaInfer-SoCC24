...
trait Container {
  ...
  def run(parameters: JsValue,
    ...
    val parameterWrapper = JsObject("value" -> parameters)    //? What is body？
    val body = JsObject(parameterWrapper.fields ++ environment.fields) //?
    val start =
      transid.started(
        this,
        LoggingMarkers.INVOKER_ACTIVATION_RUN,
        s"sending Run arguments to $actionName at $id $addr.  AND, BODY IS : $body, TIMEOUT IS : $timeout",
        logLevel = InfoLevel)
  ...
  /** Runs code in the container. Thread-safe - caller may invoke concurrently for concurrent activation processing. */
  def load(parameters: JsValue,
           environment: JsObject,
           timeout: FiniteDuration,
           maxConcurrent: Int,
           reschedule: Boolean = false)(implicit transid: TransactionId): Future[Unit] = {
    val actionName = environment.fields.get("action_name").map(_.convertTo[String]).getOrElse("")
    ...
    val start =
      transid.started(
        this,
        LoggingMarkers.INVOKER_ACTIVATION_RUN,
        s"sending Load arguments to $actionName at $id $addr.  AND, BODY IS : $body, TIMEOUT IS : $timeout",
        logLevel = InfoLevel)


    callContainer("/load", body, timeout, maxConcurrent, retry = false, reschedule).map { result =>
      result.response.right.map { res =>
        if (res.shuttingDown) {
          throw ContainerHealthError(transid, containerId.asString)
        }
  ...
  /** Runs code in the container. Thread-safe - caller may invoke concurrently for concurrent activation processing. */
  def offload(parameters: JsValue,
              environment: JsObject,
              timeout: FiniteDuration,
              maxConcurrent: Int,
              reschedule: Boolean = false)(implicit transid: TransactionId): Future[Unit] = {
    ...
    val start =
      transid.started(
        this,
        LoggingMarkers.INVOKER_ACTIVATION_RUN,
        s"sending Offload arguments to $actionName at $id $addr.  AND, BODY IS : $body, TIMEOUT IS : $timeout",
        logLevel = InfoLevel)


    callContainer("/offload", body, timeout, maxConcurrent, retry = false, reschedule).map { result =>
      result.response.right.map { res =>
        if (res.shuttingDown) {
          throw ContainerHealthError(transid, containerId.asString)
        }
  ...
  protected def callContainer(path: String,
    ...
    http
      .post(path, body, ActivationEntityLimit.MAX_ACTIVATION_ENTITY_LIMIT,
        ActivationEntityLimit.MAX_ACTIVATION_ENTITY_TRUNCATION_LIMIT, retry, reschedule)
      .map { response =>
