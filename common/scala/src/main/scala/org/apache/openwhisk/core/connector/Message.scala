...
//import org.apache.openwhisk.core.keepAlive
...
case class ActivationMessage(override val transid: TransactionId,
                             ...
                             var preWarmParameter: Int = 10,
                             var keepAliveParameter: Int = 10,
                             var possionpara1_lambda: Double = 0,
                             var preLoadParameter: Int =1,
                             var offLoadParameter: Int =10,
                             ...
                             //添加一个keepAliveParameter，记录keep-alive window信息)
                            )
    extends Message {
...
object ActivationMessage extends DefaultJsonProtocol {
  ...
  implicit val serdes = jsonFormat17(ActivationMessage.apply)   //修改了这里，因为多了一个参数，所以把12改成13
}
